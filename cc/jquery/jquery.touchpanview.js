...
;(function($) {
	...
	$.fn.touchPanView = function( cfg ) {
		...
		var cfg = $.extend({},{
			...
			zoomStep:			10,			// used with zoom plugin to "zoom(in)" and "zoom(out)"
		...
		$(this).each(function(){
			...
			var dragEnd = function( pos ) {
				...
				console.log(css);
				jQuery('#cropleft').val(Math.round(css.left));
				jQuery('#croptop').val(Math.round(css.top));
	...
	$.fn.touchPanZoom = function( zoom, animate ) {
		...
		var updatePins = function(animate) {
			...
			for ( var i=0; i<wdg.cfg.pins.length; i++ ) {
				...
				jQuery('#cropleft').val(Math.round(css.left));
				jQuery('#croptop').val(Math.round(css.top));
				jQuery('#cropheight').val(Math.round(css.height));
				jQuery('#cropwidth').val(Math.round(css.width));
